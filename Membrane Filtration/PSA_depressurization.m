...
%
function yt = PSA_depressurization(~,Y) % PSA_depressurization(t,Y)
%
% parameters shared with the ODE routine
%
global Mg1 Mg2 De1 De2 taup dYbardZbarDifferentialMethod rhog0 T0 ncall Qa Qd n dzbar dtbar epsilon epsilonbar k1 k2 k3 k4 rhob dp L y1d Qin Pd tc tt Pmax Pmin Tmax Tmin Tref MassAxial dtbara dtbarp dtbardep dtbard miu ta tp td tdep
%
% preallocating
%
Y1bar = zeros(1,n);
...
if (ncall ~= 0) % not initial condition
    i = n; % Only use boundary conditions at the adsorption outlet
    y1bar(i) = y1d;
    Y1bar(i) = y1d*(rhog0*L/(Qa*tt));
end
...
Yt(0*n+n) = 0; % because Y1bar(n) = y1d*rhog(n)
%
yt = Yt';
