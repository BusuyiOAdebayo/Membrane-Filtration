...
%
function yt = PSA_pressurization(~,Y) % PSA_pressurization(t,Y)
%
...
% preallocating
%
Y1bar = zeros(1,n); % Fluid concentration of species 1
n1bar = zeros(1,n); % Solid concentration (specific molar amount adsorbed) of species 1
n2bar = zeros(1,n); % Solid concentration (specific molar amount adsorbed) of species 2
Pbar = zeros(1,n); % Pressure
y1bar = zeros(1,n); % Fluid molar fraction of species 1
Mg = zeros(1,n); % Fluid molar mass
thetabar = zeros(1,n);
rhogbar = zeros(1,n);
n1eqmbar = zeros(1,n); % Equilibrium molar amount adsorbed of species 1
n2eqmbar = zeros(1,n); % Equilibrium moalr amount adsorbed of species 2
Qbar = zeros(1,n);
usbar = zeros(1,n); 
...
m = 0.0175e-4;
c = 0.15e-4;
kp1 = m*(1-epsilon)*rhob/epsilon^3/dp;
kp2 = c*(1-epsilon)^2*miu/epsilon^3/dp^2;
K = 1e-7;
for i = 1
    dPbardtbar(i) = K/miu/epsilon*ta*(Pbar(i+2)*(Pbar(i+2)-Pbar(i+1))-Pbar(i)*(Pbar(i+1)-Pbar(i)))*(Pmax-Pmin)/(dz)^2;
end
for i = 2:(n-1)
    dPbardtbar(i) = K/miu/epsilon*ta*(Pbar(i+1)*(Pbar(i+1)-Pbar(i))-Pbar(i-1)*(Pbar(i)-Pbar(i-1)))*(Pmax-Pmin)/(dz)^2;
end
for i = n
    dPbardtbar(i) = K/miu/epsilon*ta*(Pbar(i)*(Pbar(i)-Pbar(i-1))-Pbar(i-1)*(Pbar(i)-Pbar(i-1)))*(Pmax-Pmin)/(dz)^2;
end
%
dPbardtbar(1) = 0;
%
