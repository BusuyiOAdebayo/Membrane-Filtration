% SPTFF Model
function dcTFFdt = continousSPTFFModel(~,cTFF,noOfComps,cTFFfeed,vdotfeed,Patm,L,Nz,d,H,k,rho,mu,De)
% cTFF and thus dcTFFdt here are column vectors of length noOfComps*Nz
% function dcTFFdt = SPTFFModel(~,cTFF,theta)
% noOfComps = theta(1)
% cTFFfeed = theta(noOfComps);
...
c_factor = 7; % cTFF(Nz)/cTFFfeed(9); 

% Step for position
z = linspace(0,L,Nz);
dz = z(2)-z(1);

% Initialize dcTFFdt, P_axi and vdot
...
% Right-interior equations for P_axi and vdot
for i = 2:Nz
    dP_axidz(i) = -(128*mu/pi/d^4)*vdot(i)+rho*g; % For cylindrical channel
    ...
    dvdotdz(i) = -pi*d*Jr(i); % For cylindrical channel
